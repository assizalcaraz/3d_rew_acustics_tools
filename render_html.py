...
<style>
  ...
  #views button.on{background:#1f2a44;color:#fff;border-color:#1f2a44}
  #views .mode-sep{width:1px;align-self:stretch;background:#cfd4de;margin:0 2px}
  /* --- Relevamiento REW --- */
  #rew-panel{
    display:none;position:absolute;top:52px;left:12px;bottom:12px;width:min(380px,calc(100% - 24px));
    background:rgba(255,255,255,.97);border:1px solid #dfe2e8;border-radius:12px;
    box-shadow:0 4px 18px rgba(0,0,0,.10);z-index:6;overflow:hidden;flex-direction:column}
  html.rew-mode #rew-panel{display:flex}
  html.rew-mode #info,html.rew-mode #inspector,html.rew-mode #hint,html.rew-mode #side{display:none!important}
  html.rew-mode #app{display:block}
  html.rew-mode #stage{position:absolute;inset:0}
  .rew-head{display:flex;align-items:center;justify-content:space-between;gap:8px;
    padding:10px 12px;border-bottom:1px solid #e4e7ee;background:#eef2fb;flex:none}
  .rew-head strong{font-size:13px;color:#1f2a44}
  .rew-head-actions{display:flex;gap:4px;flex-wrap:wrap}
  .rew-head-actions button,.rew-btn-file,.rew-actions button{
    background:#fff;border:1px solid #cfd4de;border-radius:7px;padding:4px 8px;
    font-size:11px;cursor:pointer;color:#1f2a44}
  .rew-btn-file{display:inline-flex;align-items:center;cursor:pointer;overflow:hidden;position:relative}
  .rew-btn-file input{position:absolute;inset:0;opacity:0;cursor:pointer}
  #rew-sections{overflow-y:auto;padding:8px 10px 16px;flex:1}
  .rew-sec{border:1px solid #e4e7ee;border-radius:8px;margin-bottom:8px;background:#fff}
  .rew-sec summary{cursor:pointer;padding:8px 10px;font-size:12px;font-weight:600;color:#1f2a44;list-style:none}
  .rew-sec summary::-webkit-details-marker{display:none}
  .rew-sec summary::before{content:'▸ ';color:#8891a3}
  .rew-sec[open] summary::before{content:'▾ '}
  .rew-sec-body{padding:0 10px 10px;font-size:12px}
  .rew-grid{display:grid;grid-template-columns:1fr 1fr;gap:6px}
  .rew-field{display:flex;flex-direction:column;gap:2px;font-size:11px;color:#445}
  .rew-field.pending input{border-color:#e0b060;background:#fffbf3}
  .rew-field input{border:1px solid #cfd4de;border-radius:6px;padding:5px 7px;font-size:12px}
  .rew-hint{color:#a07830;font-style:normal;font-size:10px}
  .rew-card{border:1px dashed #d7dbe4;border-radius:8px;padding:8px;margin-bottom:8px}
  .rew-card-t{font-weight:600;margin-bottom:6px;color:#1f2a44;font-size:12px}
  .rew-card-t em{font-weight:400;color:#8891a3}
  .rew-calc{margin-top:8px;padding:8px;border-radius:8px;background:#f4f6fa;font-size:11px;line-height:1.45;color:#333}
  .rew-calc.ok{background:#e8f6ee;border:1px solid #b7dfc4}
  .rew-calc.warn{background:#fff6e8;border:1px solid #f0d2a0}
  .rew-calc.info{background:#eef2fb;border:1px solid #cfd4de}
  .rew-calc.calc{background:#f4f6fa;border:1px solid #e4e7ee}
  .rew-ok{color:#1a7a3c}
  .rew-warn{color:#a06010}
  .rew-muted{color:#8891a3}
  .rew-badge{display:inline-block;padding:1px 7px;border-radius:999px;font-size:10px;font-weight:700}
  .rew-badge.ok{background:#d8f0e2;color:#1a7a3c}
  .rew-badge.no{background:#f0e6d8;color:#8a5a20}
  .rew-table{width:100%;border-collapse:collapse;font-size:11px}
  .rew-table td,.rew-table th{border-bottom:1px solid #eef0f4;padding:4px 3px;text-align:left}
  .rew-table.compact td{font-variant-numeric:tabular-nums;font-family:ui-monospace,Menlo,monospace;font-size:10px}
  .rew-actions{display:flex;gap:6px;margin:8px 0;flex-wrap:wrap}
  .rew-checklist{display:flex;flex-direction:column;gap:4px}
  .rew-check{display:flex;align-items:flex-start;gap:6px;font-size:11px;cursor:pointer}
  #rew-plano-wrap{
    display:none;position:absolute;top:52px;right:12px;bottom:12px;width:min(420px,calc(100% - 420px));
    min-width:260px;background:rgba(255,255,255,.97);border:1px solid #dfe2e8;border-radius:12px;
    box-shadow:0 4px 18px rgba(0,0,0,.10);z-index:6;overflow:auto;padding:10px 12px;box-sizing:border-box}
  html.rew-mode #rew-plano-wrap{display:block}
  .rew-plano-bar{display:flex;justify-content:space-between;gap:8px;font-size:12px;margin-bottom:6px;color:#1f2a44;font-weight:600}
  #rew-plano{width:100%;height:auto;background:#fff;border:1px solid #e4e7ee;border-radius:8px}
  .rew-leyenda{display:flex;flex-wrap:wrap;gap:8px;margin-top:8px;font-size:10px;color:#445}
  .rew-leg{display:inline-flex;align-items:center;gap:4px}
  .rew-leg i{display:inline-block;width:8px;height:8px;border-radius:50%}
  .rew-leg.muted{color:#8891a3}
  @media (max-width: 900px) {
    #rew-panel{width:calc(100% - 24px);bottom:auto;max-height:48vh}
    #rew-plano-wrap{top:auto;left:12px;right:12px;bottom:12px;width:auto;max-height:42vh}
  }
</style>
</head>
<body>
<div id="app">
  <div id="stage">
    ...
    <div id="views">
      <button type="button" data-mode="tratamiento" class="on" title="Visor 3D de tratamiento">Tratamiento</button>
      <button type="button" data-mode="rew" title="Relevamiento REW de campo">Relevamiento REW</button>
...
(function () {
  ...
  function goView(v) {
    if (v === 'zoomin' || v === 'zoomout') {
      var dir = new THREE.Vector3().subVectors(controls.target, cam.position);
      var len = dir.length();
  ...
  document.querySelectorAll('#views button[data-v]').forEach(function (b) {
    b.addEventListener('click', function () { goView(b.dataset.v); });
  });

  /* --- Marcadores M1–M5 (coords app) ligados al operador REW --- */
  var micGroup = new THREE.Group();
  micGroup.visible = true;
  escena.add(micGroup);
  var micMeshes = {};
  var micLabs = {};
  var MIC_IDS = ['M1', 'M2', 'M3', 'M4', 'M5'];
  var MIC_COLORS = { M1: 0x1f2a44, M2: 0x16a2a2, M3: 0x16a2a2, M4: 0x16a2a2, M5: 0x16a2a2 };
  MIC_IDS.forEach(function (id) {
    var geo = new THREE.SphereGeometry(id === 'M1' ? 0.06 : 0.045, 16, 12);
    var mesh = new THREE.Mesh(geo, new THREE.MeshStandardMaterial({
      color: MIC_COLORS[id], roughness: 0.5, metalness: 0.1
    }));
    mesh.visible = false;
    micGroup.add(mesh);
    micMeshes[id] = mesh;
    var lab = mkLbl(id, id === 'M1' ? '#1f2a44' : '#0f7878', true);
    lab.style.display = 'none';
    micLabs[id] = lab;
    followLabs.push({ el: lab, mesh: mesh, mic: true });
  });

  function updateMicMarkers(appMics) {
    MIC_IDS.forEach(function (id) {
      var p = appMics && appMics[id];
      var mesh = micMeshes[id];
      var lab = micLabs[id];
      if (!p) {
        mesh.visible = false;
        lab.style.display = 'none';
  ...
  if (window.RewApp) {
    RewApp.init({
      getRoomDims: function () { return { L: L, W: W, H: H }; },
      onMicsChange: updateMicMarkers,
      onModeChange: function (m) {
        var showMics = true;
        MIC_IDS.forEach(function (id) {
          if (!micMeshes[id].visible) return;
          micLabs[id].style.visibility = showMics ? 'visible' : 'hidden';
        });
        if (m === 'rew') {
          /* Panel/plano ocupan el foco; 3D queda de fondo atenuado */
  ...
  function refreshLabels() {
    ...
    followLabs.forEach(function (lb) {
      if (lb.mic && (!lb.mesh.visible || document.documentElement.classList.contains('rew-mode'))) {
...
print("render.html regenerado (tratamiento + relevamiento REW)")
